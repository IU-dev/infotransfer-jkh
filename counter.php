...
<center>
    ...
    <?php if (isset($_SESSION['logged_in']) && isset($_GET['id'])) : ?>
        ...
        if ($info['client_id'] != $user->id) {
            echo 'Попытка обхода - Вы пытаетесь получить информацию по чужому счетчику.';
        } else {
            if ($info['type'] == "EE") echo '<h3>Счетчик электроэнергии ' . $info['model'] . '&nbsp;&nbsp;&nbsp;<span class="badge badge-secondary">CTR-' . $info['id'] . '</span></h3>';
            else if ($info['type'] == "W") echo '<h3>Счетчик воды ' . $info['model'] . '&nbsp;&nbsp;&nbsp;<span class="badge badge-secondary">CTR-' . $info['id'] . '</span></h3>';
