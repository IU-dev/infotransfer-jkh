<?php
	...
	class UserTools {
		...
		public function add_points($username, $who, $num, $comment){
				...
				$data1['ot'] = "'Система'";
				$data1['text'] = "'Пользователь ".$who." добавил Вам ".strval($num)." баллов (было ".strval($now_points).", стало ".strval($up_points).")'";
		...
		public function rem_points($username, $who, $num, $comment){
				...
				$data1['datetime'] = "'".date("Y-m-d H:i:s",time())."'";
				$data1['ot'] = "'Система'";
				$data1['text'] = "'Пользователь ".$who." снял Вам ".strval($num)." баллов (было ".strval($now_points).", стало ".strval($up_points).")'";
				$db->insert($data1, 'logs');
				return true;
		}
